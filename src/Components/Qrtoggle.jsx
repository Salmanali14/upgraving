...
export default function Qrtoggle({ profilelockmode,isOnline, toggleOnlineStatus }) {
  ...
  const updateOnlineStatus = (newOnlineStatus) => {
    ...
    .then(() => {
      ...
      if (newOnlineStatus) {
        toast.success(t('Offline mood is enable'));
      } else {
        toast.info(t('Offline mood is disable'));
      }
    })
    .catch((error) => {
    ...
    <div>
      <IOSSwitch
        onChange={(event) => {
          ...
          toggleOnlineStatus(); // Assuming this function is responsible for local state update
          updateOnlineStatus(newOnlineStatus); // Update online status in Firebase
        }}
        checked={isOnline}
      />
    </div>
