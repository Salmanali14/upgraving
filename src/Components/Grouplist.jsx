...
export default function Grouplist() {
  ...
  const [searchQuery, setSearchQuery] = useState(''); const [ourInfo, setOurInfo] = useState({ fullname: '', location: '', jobtitle: '', company: '', bio: '' });
  const [coverImage, setCoverImage] = useState('');
  const [logoImage, setLogoImage] = useState('');
  const [showProfileUpload, setShowProfileUpload] = useState(true);
  const profileInputRef = useRef(null);
  const [groupImage, setProfileImage] = useState('');
  const [group,setGroup] = useState({ fullname: '',groupImage:''});
...
/>
  ...
  .map((connection, index) => (
      <div key={index} className='single-con6'>
        <span style={{ display: 'flex', alignItems: 'center', width: '62%', marginTop: '5px' }}>
          <Link to={`/singlegroup1/${connection.id}`}>
            <img src={connection.groupImage ? connection.groupImage : prfl} alt={`Group ${index + 1}`} />{'\u00A0'}
          </Link>
          <Link to={`/singlegroup1/${connection.id}`}>
            <h2 id='icon-group-3'>{truncateText(connection.fullname,15)}</h2>
          </Link>
        </span>
        <div className='single-con-about6' style={{ margin: '0px' }}>
          <div className='single-con-btn6'>
            <Link to={`/singlegroup1/${connection.id}`}>
              <button id='more-con-btns-06'>{t("Open")}</button>
            </Link>
      ...
      >
        ...
        >
          ...
          <div className='Create-group-popup'>
            ...
            <img src={groupImage ? groupImage : prfl } />
              {showProfileUpload && (
        ...
        <input type='text' name='fullname' placeholder='Full Name' value={group.fullname} onChange={handleInputChange} />
